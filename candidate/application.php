...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Step 2: Handle file upload for resume
    $resume = $_FILES['resume']['name'];
    $resume_tmp = $_FILES['resume']['tmp_name'];
    $resume_path = 'uploads/' . $resume;
    move_uploaded_file($resume_tmp, $resume_path);

    // Step 3: Save application data into the database
    ...
    if (mysqli_query($conn, $sql)) {
        ...
        // Step 4: Fetch company email using job_id
        ...
        if ($job && !empty($job['email'])) {
            ...
            // Step 5: Send an email to the company
...
function sendApplicationToCompany($full_name, $email, $job_id, $resume_path, $cover_letter, $linkedin_profile, $companyEmail) {
    try {
        ...
        $mail->SMTPAuth   = true;
        $mail->Username   = 'jobboard2025@gmail.com';  // Replace with your email
        $mail->Password   = 'lkwi qqcd qxoy ethr';      // Replace with your app password
        $mail->SMTPSecure = PHPMailer::ENCRYPTION_STARTTLS;
        $mail->Port       = 587;

        $mail->setFrom('jobboard2025@gmail.com', 'Job Board');
        $mail->addAddress($companyEmail); // Company's email
        $mail->isHTML(true);
                        ...
                        <form action="application.php" method="POST" enctype="multipart/form-data" id="job-application-form">
                            ...
                            <div class="form-group mb-4">
                                ...
                                <select class="form-control" id="job_id" name="job_id" required>
                                    <?php
                                    // Connect to the database and fetch job listings
